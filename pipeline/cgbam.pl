...
$0 tablebam --bam <bamFile> --easycoord --bed
  ...
  --samtoolspath STR  samtools path or singularity image containing samtools
";
  ...
  open INFILE, "$samtoolspath view -h -F 0x700 $bamFile | " || die "Fail to open $bamFile\n$!\n";
  my %excludeIds = ();
  foreach my $excludeIds (@excludeIds) {
    foreach my $excludeId (split(/[,;]/,$excludeIds)) {
      $excludeIds{$excludeId} = 1; 
    }
  }
  my @readids = ();
  my %reads = ();
  ...
  if (scalar(@readids)>0) {
    if (0==$bedFormat) {
      my @cols = ('readId','flag','mapq','contig','contigStart','numAlignments','alignId','readLen','refLen','overlapNote','readStartEndLen','refStartEndLen','de','cigar');
      print join("\t", @cols), "\n";
    }
  }
  my %selectedReadIds = ();
  ...
  foreach my $readId (@readids) {
    ...
    for(my $i=0; $i<$numRecords; ++$i) {
      ...
      if (0==$bedFormat) {
        printf "%s\t%d\t%d\t%s\t%d\t%d\t%d\t%d\t%d\t%s\tread:%d-%d[%d]\t%s:%d-%d[%s%d]\t%s\t%s\n",
        ...
        push @cols, sprintf("%s_%dof%d:%d-%d[%d]_de%s", $recordRef->{read}, $alignId, $numAlignments, $recordRef->{read_start0}+1, $recordRef->{read_end1}, $recordRef->{read_end1}-$recordRef->{read_start0}, $recordRef->{de});
...
sub getSortableCGId {
  ...
  if ($id=~/^(.+)(\.cG[M]{0,1})(\d+)\.(\d+)\.(\d+)/) {
    my $literal1 = $1;
    my $literal2 = $2;
    my $val1 = $3;
    my $val2 = $4;
    my $val3 = $5; # format as two digits

    return sprintf("%s%s%d.%d.%02d", $literal1, $literal2, $val1, $val2, $val3);
  }
...
";
  ...
  open INFILE, "$samtoolspath view -h -F 0x700 $bamFile | " || die "Fail to open $bamFile\n$!\n";
  open OUTFILE, "| $samtoolspath view -b -o $queryBam -" || die "Fail to open $queryBam\n$!\n";
  my @readids = ();
  my %reads = ();
  my %refLens = ();
  my $numRefs = 0;
  while (<INFILE>) {
    ...
    next if (($record{read_end1}-$record{read_start0})<$minlen);
  ...
  # generate bam by coordinates
  my $coordBam = sprintf("%s.rep.anno.bam", $bamPrefix);
  my @commands = ($samtoolspath, 'sort', '-o', $coordBam, $queryBam);
  ...
  # index coord bam
  @commands = ($samtoolspath, 'index', $coordBam);
