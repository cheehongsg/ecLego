...
initsubpop   : initialize subpopulation for checks
harvestresult: harvest results from iterative workflow
...
my $G_SAMTOOLS_SIF = "$G_ECLEGO_PIPELINE_PATH/samtools_1.19.sif";
...
if ('sievegraph' eq $command) {
	...
	initSubgraph();
} elsif ('initsubpop' eq $command) {
	initSubpopulation();
} elsif ('harvestresult' eq $command) {
	harvestResult();
} else {
	print $G_USAGE;
}
...
sub get_samtools_cmd {
    my ($sif) = @_;

    my $local_path = `which samtools 2>/dev/null`;
    chomp($local_path);
    return "samtools" if ($local_path && -x $local_path);
    return "singularity exec $sif samtools" if ($sif && -e $sif);
    die "ERROR: 'samtools' not found in PATH and no Singularity image found at: $sif\n";
}
...
sub graphBamFile {
  my ($samtoolspath, $bamFile, $graphRef, $graphEdgeType, $cov2cn, $file) = @_;
  ...
  # index new bam file
  open INFILE, "$samtoolspath view -h $bamFile | " || die "Fail to open $bamFile\n$!\n";
  open OUTFILE, "|$samtoolspath view -b - >  $file" || die "Fail to open $file\n$!\n";
  while (<INFILE>) {
    if (/^@/) {
      print OUTFILE $_;
    } else {
  ...
  close OUTFILE;
  close INFILE;
  system("$samtoolspath index $file");
}
...
";
  ...
  if (defined $samtoolspath) {
      $samtoolspath = get_samtools_cmd($samtoolspath);
  } else {
      $samtoolspath = get_samtools_cmd($G_SAMTOOLS_SIF);
  }

  my %graph = ();
  readGVGraphFile($gvFile, $subsample, \%graph);
  ...
  if (defined $bamFile) {
    ...
    graphBamFile($samtoolspath, $bamFile, $graph{disjointcyclicGraphs}, $graph{edgeType}, $cov, $oFile);
  }
}

sub parseSummaryFile {
  my ($file, $minKb, $summaryRef) = @_;

  $summaryRef->{file} = $file;
  $summaryRef->{minKb} = $minKb;

  open INFILE, $file || die "Fail to open $file\n$!\n";

  my @putatives = ();
  $summaryRef->{putatives} = \@putatives;
    ...
    if (/^\/\//) {
      ...
      if (0==$numCols) {
        if (/^subgraphId\t/) {
          ...
          @cols = ('subgraphId', 'numLinearVertices', 'numLinearEdges', 'numVertices', 'numEdges');
          push @cols,	'num_0_chrom', 'kbp_0_chrom', 'avecov_0_chrom';
          push @cols,	'num_1_low_amp', 'kbp_1_low_amp', 'avecov_1_low_amp';
          push @cols, 'num_2_med_amp', 'kbp_2_med_amp', 'avecov_2_med_amp';
          push @cols, 'num_3_high_amp', 'kbp_3_high_amp', 'avecov_3_high_amp';
          push @cols,	'vertices', 'edges';
          push @cols, 'cov_0_chrom', 'len_0_chrom';
          push @cols,	'cov_1_low_amp', 'len_1_low_amp';
          push @cols, 'cov_2_med_amp', 'len_2_med_amp';
          push @cols,	'cov_3_high_amp', 'len_3_high_amp';
          push @cols,	'linearVertices', 'linearEdges';
          $numCols = scalar(@cols);
          $dataLine = 1;
        }
  ...
  $summaryRef->{fastq} = $fastq;
  $summaryRef->{fastqDir} = $fastqDir;
  $summaryRef->{sample} = $sample;
  $summaryRef->{numSubgraphs} = $numSubgraphs;

  my @bits = split(/\//, $outputPrefix);
  my $summaryFile = pop @bits;
  my $summaryDir = join("\/", @bits);
  my $summaryParentFolderName = pop @bits;
  $summaryParentFolderName = pop @bits if ('assembly_sieved_graphs' eq $summaryParentFolderName);
  $summaryRef->{summaryFile} = $summaryFile;
  $summaryRef->{summaryDir} = $summaryDir;
  ($summaryRef->{dicncov}) = $summaryRef->{summaryFile} =~ /\.cn(\d+)\./;
  ($summaryRef->{seedext}) = $summaryParentFolderName =~ /\.ci(\d+)/;
}
...
";
  ...
  if ($step ne 'refine' && $step ne 'assess') {
    die "Invalid step! $step\n";
  }

  my %summary = ();
  parseSummaryFile ($summaryFile, $minKb, \%summary);

  # sort and start with the largest (then higher copy number)
  my @putatives = ();
  push @putatives, @{$summary{putatives}};
  @putatives = sort { $b->{_sort_value}<=>$a->{_sort_value} } @putatives;
  ...
  print "#####\n";
  if ('refine' eq $step) {
    print "# Refinement script for putative circular genomes using draft assembly\n";
  } elsif ('assess' eq $step) {
    print "# Assessment script for putative circular genomes using draft assembly\n";
  }
  print "#####\n";
  print "\n";
  printf "# summary file: %s\n", $summaryFile;
  printf "# %d/%d of the subgraphs contain putative circular genomes.\n", scalar(@putatives), $summary{numSubgraphs};
  my $numPutatives = scalar(@putatives);
  if ($numPutatives>0) {
    if ('' ne $summary{fastqDir}) {
      printf "cd %s \n\n", $summary{fastqDir};
    } else {
      print "# cd # unavailable; relative path used\n\n";
    }
  }
  for(my $i=0; $i<$numPutatives; ++$i) {
    ...
    if ('refine' eq $step) {
      printf "%secLego3.refine.sh \\\n", $G_LOCAL_DIR;
    } elsif ('assess' eq $step) {
      printf "%secLego3.assess.refined.sh \\\n", $G_LOCAL_DIR;
    }
    printf "%s \\\n", $summary{fastq};
    printf "%s \\\n", $genome;
    printf "%d", $subgraphRef->{subgraphId};
    if (defined $cov) {
      printf " %d", $cov;
      if (defined $seedExt) {
        printf " %d", $seedExt;
    ...
    printf " 2>&1 | tee %s.ecLego3.%d_%d_subgraph_%d", 
      $summary{sample}, ($i+1), $numPutatives, $subgraphRef->{subgraphId};
    if ('refine' eq $step) {
      print ".refine.run.log\n", 
    } elsif ('assess' eq $step) {
      print ".assess.refined.run.log\n", 
    }
    print "\n";
    print "# END of script\n\n";
  }
...
sub initSubpopulation {
  my $G_USAGE = "
$0 initsubpop --summary <summaryFile>
  --summary STR       summary file
";
  ...
  # extract refined subgraph IDs from assembly_sieved_graphs
  my $numPutatives = scalar(@putatives);
  for(my $i=0; $i<$numPutatives; ++$i) {
    my $subgraphRef = $putatives[$i];

    my $subgraphFolderName = sprintf("%s.cn%d.disjointcyclic.gv.subgraph_%d", 
      $summary{sample}, (! defined $cov) ? $summary{dicncov} : $cov, $subgraphRef->{subgraphId});
    my $refinedAssemblyFolderName = sprintf("%s.subgraph_%d_candidate_assembly.ci%d", 
      $summary{sample}, $subgraphRef->{subgraphId}, (! defined $seedExt) ? $summary{seedext} : $seedExt);
    my $sievedFolderName = "assembly_sieved_graphs";
    my $refinedSummaryDir = sprintf("%s/%s/%s/%s", 
      $summary{summaryDir}, $subgraphFolderName, $refinedAssemblyFolderName, $sievedFolderName);

    my $refinedSummaryFile = sprintf("%s/%s.sG%d.cn%d.disjointcyclic.gv.min%dkb.overview.xls", 
      $refinedSummaryDir, 
      $summary{sample}, $subgraphRef->{subgraphId}, (! defined $cov) ? $summary{dicncov} : $cov, $minKb);
    if (-f $refinedSummaryFile) {
      my %refinedSummary = ();
      parseSummaryFile ($refinedSummaryFile, $minKb, \%refinedSummary);
      $subgraphRef->{refinedSummary} = \%refinedSummary;
    }
  }
  ...
  print "\n";
  print "#####\n";
  print "# Subpopulations generation script\n";
  print "#####\n";
  print "\n";
  printf "# summary file: %s\n", $summaryFile;
  printf "# %d/%d of the subgraphs contain putative circular genomes.\n", $numPutatives, $summary{numSubgraphs};
  if ($numPutatives>0) {
    if ('' ne $summary{fastqDir}) {
      printf "cd %s \n\n", $summary{fastqDir};
    } else {
  ...
  for(my $i=0; $i<$numPutatives; ++$i) {
    ...
    my @refinedPutatives = ();
    push @refinedPutatives, @{$subgraphRef->{refinedSummary}->{putatives}};
    @refinedPutatives = sort { $b->{_sort_value}<=>$a->{_sort_value} } @refinedPutatives;
    my $numRefinedPutatives = scalar(@refinedPutatives);
    ...
    printf "# %d. subgraph_%d (%sL: %s kbp, %sM: %s kbp, %sH: %s kbp)\n", 
      ...
      ($subgraphRef->{kbp_2_med_amp}>=$minKb) ? '*' : '', $subgraphRef->{kbp_2_med_amp}, 
      ($subgraphRef->{kbp_3_high_amp}>=$minKb) ? '*' : '', $subgraphRef->{kbp_3_high_amp};
    if (0==$numRefinedPutatives) {
      print "# NO refined subgraphs; nothing to process\n";
    } else {
      printf "# refined subgraphs: %d\n", $numRefinedPutatives;
      my @values = ();
      grep { push @values, sprintf("%d (%d)", $_->{subgraphId}, $_->{_sort_value}) } @refinedPutatives;
      printf "# refined subgraph ids: %s\n", join(",", @values);
      print "\n";
    }

    for(my $j=0; $j<$numRefinedPutatives; ++$j) {
      # TODO: 
      my $refinedSubGraphRef = $refinedPutatives[$j];
      printf "# %d. subgraph_%d (%sL: %s kbp, %sM: %s kbp, %sH: %s kbp)\n", 
        ($i+1), $subgraphRef->{subgraphId}, 
        ($subgraphRef->{kbp_1_low_amp}>=$minKb) ? '*' : '', $subgraphRef->{kbp_1_low_amp}, 
        ($subgraphRef->{kbp_2_med_amp}>=$minKb) ? '*' : '', $subgraphRef->{kbp_2_med_amp}, 
        ($subgraphRef->{kbp_3_high_amp}>=$minKb) ? '*' : '', $subgraphRef->{kbp_3_high_amp};
      printf "# %d.%d refined_subgraph_%d (%sL: %s kbp, %sM: %s kbp, %sH: %s kbp)\n", 
        ($i+1), ($j+1), $refinedSubGraphRef->{subgraphId}, 
        ($refinedSubGraphRef->{kbp_1_low_amp}>=$minKb) ? '*' : '', $refinedSubGraphRef->{kbp_1_low_amp}, 
        ($refinedSubGraphRef->{kbp_2_med_amp}>=$minKb) ? '*' : '', $refinedSubGraphRef->{kbp_2_med_amp}, 
        ($refinedSubGraphRef->{kbp_3_high_amp}>=$minKb) ? '*' : '', $refinedSubGraphRef->{kbp_3_high_amp};

      printf "%secLego3.generate.subpop.sh \\\n", $G_LOCAL_DIR;
      printf "%s \\\n", $summary{fastq};
      printf "%s \\\n", $genome;
      printf "%d", $subgraphRef->{subgraphId};
      printf " %d", $refinedSubGraphRef->{subgraphId};
      if (defined $cov) {
        printf " %d", $cov;
        if (defined $seedExt) {
          printf " %d", $seedExt;
      ...
      printf " 2>&1 | tee %s.ecLego3.%d_%d_subgraph_%d.subpop.run.log\n", 
        $summary{sample}, ($i+1), $numPutatives, $subgraphRef->{subgraphId};
      print "\n";
    }
...
sub harvestResult {
  my $G_USAGE = "
$0 harvestresult --summary <summaryFile>
  --summary STR       summary file
";
  ...
  # TODO: harvest useful results into a new folder for downstream analysis and manual inspection
  my %summary = ();
  parseSummaryFile ($summaryFile, $minKb, \%summary);
  ...
  print "\n";
  print "#####\n";
  print "# harvest the results for downstream analysis and manual inspection\n";
  print "#####\n";
  print "\n";
  printf "# summary file: %s\n", $summaryFile;
  printf "# %d/%d of the subgraphs contain putative circular genomes.\n", $numPutatives, $summary{numSubgraphs};
  ...
  print "# Packaging top-level data\n";
  printf "mkdir -p %s\n", $resultPath;
  # metrics
  printf "rsync -avLP *.metrics %s/\n", $resultPath;
  print "\n";

  my $destDir = sprintf("%s/exec", $resultPath);
  print "# Packaging scripts and logs\n";
  printf "mkdir -p %s\n", $destDir;
  # bash scripts
  printf "rsync -avLP *.sh %s/\n", $destDir;
  $destDir = sprintf("%s/exec/logs", $resultPath);
  printf "mkdir -p %s\n", $destDir;
  # run logs
  printf "rsync -avLP *.log %s/\n", $destDir;
  print "\n";
  print "\n";


  $destDir = sprintf("%s/draft_genomes", $resultPath);
  print "# Packaging Draft genomes\n";
  printf "# Source: %s\n", $summary{summaryDir};
  printf "# Destination: %s\n", $destDir;
  printf "mkdir -p %s\n", $destDir;
  # assembly_graph.gv, assembly_graph.gfa, assembly.fasta, assembly_info.txt
  printf "rsync -avLP %s/assembly{_graph.gv,_graph.gfa,.fasta,_info.txt} %s/\n", $summary{summaryDir}, $destDir;
  printf "rsync -avLP %s/params.json %s/\n", $summary{summaryDir}, $destDir;
  printf "rsync -avLP %s/flye.log %s/\n", $summary{summaryDir}, $destDir;
  # <sample>.cn4.disjointcyclic.bam, <sample>.cn4.disjointcyclic.bam.bai
  printf "rsync -avLP %s/*.disjointcyclic.bam* %s/\n", $summary{summaryDir}, $destDir;
  # <sample>.cn4.disjointcyclic.gv{,.details,.graphvizonline.html,.min25kb.overview.xls,.overview.xls}
  printf "rsync -avLP %s/*.disjointcyclic.gv{,.details,.graphvizonline.html,.min25kb.overview.xls,.overview.xls} %s/\n", $summary{summaryDir}, $destDir;
  # <sample>.cn4.disjointcyclic.gv.subgraph_*{,.gv,.readids,.fasta}
  printf "rsync -avLP %s/*.disjointcyclic.gv.subgraph_*.{gv,readids,fasta} %s/\n", $summary{summaryDir}, $destDir;
  print "\n";
  print "\n";


  # TODO: $summary{fastq}
  # TODO: $summary{summaryDir} 
  # printf "mkdir -p %s/refined_genomes\n", $resultPath;
  ...
  for(my $i=0; $i<$numPutatives; ++$i) {
    ...
    printf "# %d. DRAFT - subgraph_%d (%sL: %s kbp, %sM: %s kbp, %sH: %s kbp)\n", 
      ($i+1), $subgraphRef->{subgraphId}, 
      ($subgraphRef->{kbp_1_low_amp}>=$minKb) ? '*' : '', $subgraphRef->{kbp_1_low_amp}, 
      ($subgraphRef->{kbp_2_med_amp}>=$minKb) ? '*' : '', $subgraphRef->{kbp_2_med_amp}, 
      ($subgraphRef->{kbp_3_high_amp}>=$minKb) ? '*' : '', $subgraphRef->{kbp_3_high_amp};
    ...
    # refined subgraphs
    for(my $j=0; $j<$numRefinedPutatives; ++$j) {
      # record the subgraph and the refined subgraph
      my $refinedSubGraphRef = $refinedPutatives[$j];
      printf "# %d.%d REFINED - refined_subgraph_%d (%sL: %s kbp, %sM: %s kbp, %sH: %s kbp)\n", 
        ($i+1), ($j+1), $refinedSubGraphRef->{subgraphId}, 
        ($refinedSubGraphRef->{kbp_1_low_amp}>=$minKb) ? '*' : '', $refinedSubGraphRef->{kbp_1_low_amp}, 
        ($refinedSubGraphRef->{kbp_2_med_amp}>=$minKb) ? '*' : '', $refinedSubGraphRef->{kbp_2_med_amp}, 
        ($refinedSubGraphRef->{kbp_3_high_amp}>=$minKb) ? '*' : '', $refinedSubGraphRef->{kbp_3_high_amp};
      printf "#     derived from %d. DRAFT - subgraph_%d (%sL: %s kbp, %sM: %s kbp, %sH: %s kbp)\n", 
        ($i+1), $subgraphRef->{subgraphId}, 
        ($subgraphRef->{kbp_1_low_amp}>=$minKb) ? '*' : '', $subgraphRef->{kbp_1_low_amp}, 
        ($subgraphRef->{kbp_2_med_amp}>=$minKb) ? '*' : '', $subgraphRef->{kbp_2_med_amp}, 
        ($subgraphRef->{kbp_3_high_amp}>=$minKb) ? '*' : '', $subgraphRef->{kbp_3_high_amp};
      ...
      # flye results
      my $srcDir = sprintf("%s/%s.cn%d.disjointcyclic.gv.subgraph_%d/%s.subgraph_%d_candidate_assembly.ci%d", 
        $summary{summaryDir}, 
        $summary{sample}, $cov, $subgraphRef->{subgraphId},
        $summary{sample}, $subgraphRef->{subgraphId}, (! defined $seedExt) ? $summary{seedext} : $seedExt);
      $destDir = sprintf("%s/refined_genomes/subgraph_%d_%d", $resultPath, $subgraphRef->{subgraphId}, $refinedSubGraphRef->{subgraphId});
      print "# Packaging Refined genomes\n";
      printf "# Source: %s\n", $srcDir;
      printf "# Destination: %s\n", $destDir;
      printf "mkdir -p %s\n", $destDir;
      # assembly_graph.gv, assembly_graph.gfa, assembly.fasta, assembly_info.txt
      ...
      # <sample>.cn4.disjointcyclic.gv{,.details,.graphvizonline.html,.min25kb.overview.xls,.overview.xls}
      printf "rsync -avLP %s/*.disjointcyclic.gv{,.details,.graphvizonline.html,.min25kb.overview.xls,.overview.xls} %s/\n", $srcDir, $destDir;
      # <sample>.cn4.disjointcyclic.gv.subgraph_*{,.gv,.readids} // exclude ,.fasta
      printf "rsync -avLP %s/*.disjointcyclic.gv.subgraph_*.{gv,readids} %s/\n", $srcDir, $destDir;
      print "\n";
      print "\n";
    }

    # sub-populations results
    for(my $j=0; $j<$numRefinedPutatives; ++$j) {
      # record the subgraph and the refined subgraph
      my $refinedSubGraphRef = $refinedPutatives[$j];
      printf "# %d.%d REFINED - refined_subgraph_%d (%sL: %s kbp, %sM: %s kbp, %sH: %s kbp)\n", 
      ...
      # flye results
      my $srcDir = sprintf("%s/%s.cn%d.disjointcyclic.gv.subgraph_%d/%s.subgraph_%d_candidate_assembly.ci%d/assembly_sieved_graphs", 
        $summary{summaryDir}, 
        $summary{sample}, $cov, $subgraphRef->{subgraphId},
        $summary{sample}, $subgraphRef->{subgraphId}, (! defined $seedExt) ? $summary{seedext} : $seedExt);
      $destDir = sprintf("%s/subpopulations/subgraph_%d_%d", $resultPath, $subgraphRef->{subgraphId}, $refinedSubGraphRef->{subgraphId});
      print "# Packaging Subpopulations\n";
      printf "# Source: %s\n", $srcDir;
      printf "# Destination: %s\n", $destDir;
      printf "mkdir -p %s\n", $destDir;
      # results
      printf "rsync -avLP %s/*.disjointcyclic.* %s/\n", $srcDir, $destDir;
      printf "rsync -avLP %s/*.CGs.* %s/\n", $srcDir, $destDir;
      printf "rsync -avLP %s/*.candidates.* %s/\n", $srcDir, $destDir;
      # fasta files
      printf "rsync -avLP %s/*.fa %s/\n", $srcDir, $destDir;
      # *.sh
      printf "rsync -avLP %s/*.sh %s/\n", $srcDir, $destDir;
      # *.log
      printf "rsync -avLP %s/*.log %s/\n", $srcDir, $destDir;
      print "\n";
      print "\n";
    }
    ...
    for(my $j=0; $j<$numRefinedPutatives; ++$j) {
      ...
      # flye results
      my $srcDir = sprintf("%s/%s.cn%d.disjointcyclic.gv.subgraph_%d", 
        $summary{summaryDir}, 
        $summary{sample}, $cov, $subgraphRef->{subgraphId});
      $destDir = sprintf("%s/triage/subgraph_%d_%d", $resultPath, $subgraphRef->{subgraphId}, $refinedSubGraphRef->{subgraphId});
      print "# Packaging triage data\n";
      printf "# Source: %s\n", $srcDir;
      printf "# Destination: %s\n", $destDir;
      printf "mkdir -p %s/{refgenome,intermediate,assembly}\n", $destDir;
      # .asref.assembly.
      printf "rsync -avLP %s/*.asref.assembly.bam* %s/assembly/\n", $srcDir, $destDir;
      printf "rsync -avLP %s/*.asref.assembly.SA.* %s/assembly/\n", $srcDir, $destDir;
      # .asref.graph_before_rr.
      printf "rsync -avLP %s/*.asref.graph_before_rr.bam* %s/intermediate/\n", $srcDir, $destDir;
      printf "rsync -avLP %s/*.asref.graph_before_rr.SA.* %s/intermediate/\n", $srcDir, $destDir;
      # reference genome
      printf "rsync -avLP %s/*.disjointcyclic.gv.subgraph_%d.assembly.{bam,bam.bai} %s/refgenome/\n", $srcDir, $subgraphRef->{subgraphId}, $destDir;
      #
      printf "rsync -avLP %s/*.disjointcyclic.gv.subgraph_%d.{bam,bam.bai,fastq} %s/refgenome/\n", $srcDir, $subgraphRef->{subgraphId}, $destDir;
      printf "rsync -avLP %s/*.disjointcyclic.gv.subgraph_%d.bam.2cn_%d.bw %s/refgenome/\n", $srcDir, $subgraphRef->{subgraphId}, $cov, $destDir;
      # bam, bam.bai, bam.2cn_4.bw
      printf "rsync -avLP %s/%s.subgraph_%d.candidates.ci%d.graph_before_rr.bam* %s/refgenome/\n", 
        $srcDir, $summary{sample}, $subgraphRef->{subgraphId}, $seedExt, $destDir;
